# financial_ai_agent.py  –  29 Apr 2025 (refined-2)
"""
...
def again(existing: bool = True) -> str:
    """Follow-up prompt."""
    if existing:
        return ("I'm your personalized JPMorgan Chase Digital Assistant — now upgraded! 🚀 "
                "I can also review your financial documents like 10-K or 10-Q reports for free. "
                "Anything else I can help with? (balance • transactions • savings — or type **upload** "
                "to attach a PDF to review your financial documents. | **exit** to leave)")
    return ("I'm your personalized JPMorgan Chase Digital Assistant — now upgraded! 🚀 "
            "I can also review your financial documents like 10-K or 10-Q reports for free. "
            "Anything else I can help with? (savings — or type **upload** "
            "to attach a PDF to review your financial documents.| **exit** to leave)")
...
def looks_like_existing(text: str) -> bool: return bool(re_existing.search(text))
def looks_like_new(text: str) -> bool:      return bool(re_new.search(text))

# ────────────────────────── chat handler ──────────────────────────
def chat(user, _history):
    global pdf_text
    ...
    try:
        ...
        if step == "zip":
            ...
            state["step"] = "ok_exist"
            return {"role": "assistant",
                    "content": f"✅ Welcome back {state['fn']}!\n\n{again(True)}"}
        ...
        if step in ("ok_exist", "ok_new"):
            ...
            if step == "ok_exist":
                ...
                if "transaction" in low or "recent" in low:
                    ...
                    tx_lines = ["Here are your last 5 transactions:"]
                    tx_lines += [f"- {r['date']}: {r['description']} (${r['amount']:.2f})" for r in rows]
            ...
            if pdf_text:
                return {"role": "assistant",
                        "content": ask_llm(msg, pdf_text) + "\n\n" + again(step == "ok_exist")}

            return {"role": "assistant", "content": again(step == "ok_exist")}
        ...
        if step == "confirm_exit":
            ...
            return {"role": "assistant", "content": "No worries – we're still connected. "
                                                + again("cid" in state)}
